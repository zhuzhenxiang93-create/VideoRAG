...
def unwrap_model_features(output: Any) -> Any:
    """Return a feature tensor across Transformers CLIP return styles."""
...
class ClipVisionRetriever(FaissDenseRetriever):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(index_dir, force_rebuild=force_rebuild)
        self.model_name = model_name
        # Keep English and Chinese CLIP indexes side by side for reproducible ablations.
        if "chinese-clip" in model_name.lower():
            self.name = "vision_dense_zh"
        self.device = device
        self._model: Any = None
    ...
    def _load(self):
        if self._model is None:
            ...
            self._model = model_class.from_pretrained(self.model_name).eval().to(actual_device)
