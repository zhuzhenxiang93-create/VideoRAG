...
<script>let current=null,openedAt=0,played=false,minTime=null,maxTime=null;const video=document.getElementById('video');video.addEventListener('play',()=>played=true);video.addEventListener('timeupdate',()=>{minTime=minTime===null?video.currentTime:Math.min(minTime,video.currentTime);maxTime=maxTime===null?video.currentTime:Math.max(maxTime,video.currentTime)});async function loadNext(){let r=await fetch('/api/next');let d=await r.json();document.getElementById('progress').textContent=`总计 ${d.progress.total}，已接受 ${d.progress.accepted}，已拒绝 ${d.progress.rejected}，待复核 ${d.progress.pending}`;if(!d.item){current=null;document.getElementById('message').textContent='当前候选均已处理。';return}current=d.item;openedAt=Date.now();played=false;minTime=null;maxTime=null;document.getElementById('editor').value=JSON.stringify(d.item.question,null,2);video.src=d.item.video_url;video.onloadedmetadata=()=>{video.currentTime=d.item.question.evidence_start||0};document.getElementById('evidence').textContent=d.item.evidence_text;document.getElementById('frames').innerHTML=d.item.frame_urls.map(x=>`<img src="${x}">`).join('');document.getElementById('message').textContent='';}async function review(action){if(!current)return;let edited;try{edited=JSON.parse(document.getElementById('editor').value)}catch(e){alert('JSON格式错误');return}let reason=document.getElementById('reason').value.trim();if(action==='reject'&&!reason){alert('拒绝时必须填写原因');return}let behavior={video_played:played,playback_ranges:minTime===null?[]:[[minTime,maxTime]],frames_present:current.frame_urls.length>0,asr_and_caption_view_present:true,review_duration_seconds:(Date.now()-openedAt)/1000};let r=await fetch('/api/review',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question_id:current.question.question_id,base_event_id:current.revision,action,reason,edited,review_behavior:behavior})});let d=await r.json();if(!r.ok){document.getElementById('message').textContent=JSON.stringify(d);return}document.getElementById('reason').value='';await loadNext();}loadNext();</script></body></html>"""
...
def create_app(
    ...
    *, allowed_video_root: Path | None = None, allowed_frame_root: Path | None = None,
    ...
    def next_candidate():
        ...
        item = next((value for value in candidates if latest.get(value["question_id"], {}).get("action") not in {"accept", "reject"}), None)
        if item is None:
